...
class AchatController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $statut = empty($champsManquants) ? 'En attente' : 'A verifier';
    ...
    */
    ...
    public function preview($id)
    {
        $achat = DB::table('factures as f')
            ...
            ->where('f.id_facture', $id)
            // Même règle de visibilité que dans show() / index().
            ->when(
                session('role') === 'admin',
                function ($query) {
                    $query->where(function ($q) {
        ...
        if (!$achat || !$achat->fichier_facture) {
            abort(404);
        }

        $path = storage_path('app/public/' . $achat->fichier_facture);
