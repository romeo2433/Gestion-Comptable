...
class VenteController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $clientNom = $this->extraireClient($texte);
            $idClient = $this->obtenirOuCreerClient($clientNom);
            ...
            if (!$clientNom) $champsManquants[] = 'client';
...
PROMPT;
    ...
    private function enregistrerFactureComplete(array $data, string $cheminFichier, int $idUtilisateur, ?int $idDepartement): int
    {
        return DB::transaction(function () use ($data, $cheminFichier, $idUtilisateur, $idDepartement) {
            ...
            if ($idClient === null) {
                throw new \Exception(
                    'Impossible de déterminer le client de la facture.'
                );
            }

            // 2. Compte de produit — récupéré ou créé automatiquement
            $idCompteProduit = $this->obtenirOuCreerCompte(
                $data['compte_produit_numero'] ?? null,
                $data['compte_produit_intitule'] ?? null
            );
            ...
            if ($montantTva > 0) {
                $idTva = DB::table('tva')->insertGetId([
                    ...
                    'montant'    => $montantTva,
                    'type_tva'   => $data['tva_type'] ?? 'TVA collectée',
            ...
            $confianceProduit = $data['compte_produit_confiance'] ?? null;
            $confianceTva = $data['compte_tva_confiance'] ?? null;

            $statut = (
                $idCompteProduit !== null &&
                $confianceProduit === 'haute' &&
                (
                    $montantTva <= 0 ||
                    (
                        $idCompteTva !== null &&
            ...
            $idFacture = DB::table('factures')->insertGetId([
                ...
                'id_utilisateur'    => $idUtilisateur,
                'id_client'         => $idClient,
                'id_compte_produit' => $idCompteProduit,
                'id_tva'            => $idTva,
     ...
     * produit comme pour le compte de TVA (les deux stockent les comptes
    ...
    private function obtenirOuCreerCompte(?string $numeroCompte, ?string $intitule, string $intituleParDefaut = 'A completer'): ?int
    {
        ...
        // (ex: 707 -> classe 7). On sécurise avec un intitulé par défaut
    ...
    /**
     * Helper pour éviter la répétition sur la création de client
     */
    private function obtenirOuCreerClient(?string $nom): ?int
    {
        if (empty($nom)) {
            return null;
        }
    ...
    */
    ...
    /**
     * Recherche le nom du client sur une facture de vente (texte brut,
     * fallback regex). Repose sur les mentions usuelles "Doit à" /
     * "Facturé à" — à ajuster selon le format réel de tes factures si le
     * taux de détection est faible.
     */
    private function extraireClient($texte)
    {
        if (preg_match('/(?:Doit\s*à|Facturé\s*à|Client\s*:?)\s*\R?\s*([^\r\n]+)/iu', $texte, $matches)) {
            return trim($matches[1]);
        }
    ...
    public function show($id)
    {
        $vente = DB::table('factures as f')
            ...
            ->select(
                'f.*',
                'cl.nom_complet as nom_client',
                DB::raw("CONCAT(c.numero_compte, ' - ', c.intitule) as compte_produit"),
        ...
        return view('ventes.show', compact(
            'vente',
            'paiements',
            'lignes'
        ));
