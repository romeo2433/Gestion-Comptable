...
@if(session('success')) <div class="alert alert-success mb-3">
{{ session('success') }} </div>
@endif

@if(session('warning')) <div class="alert alert-danger mb-3">
{{ session('warning') }} </div>
@endif

@if(session('error')) <div class="alert alert-danger mb-3">
{{ session('error') }} </div>
@endif

@if($errors->any()) <div class="alert alert-danger mb-3"> <ul>
@foreach($errors->all() as $error) <li>{{ $error }}</li>
@endforeach </ul> </div>
@endif

{{-- Import facture de vente --}}
...
<div class="card-body">
    ...
    <form action="{{ route('ventes.upload') }}"
          method="POST"
        ...
        <div class="dropzone mb-3"
            ...
            <svg width="30"
                ...
                <path d="M7 18a4 4 0 0 1-.6-7.95A5.5 5.5 0 0 1 17 9.5a4 4 0 0 1 .5 7.9"
                      stroke="currentColor"
                      stroke-width="1.4"
            ...
            <div class="dz-title">
                Cliquez pour choisir une facture de vente
            </div>
                ...
                @forelse($ventes as $vente)
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if(($vente->paiement ?? 0) > 0)

                                <form
                                    action="{{ route(
                                        'ventes.paiement.update',
                                        $vente->id_facture
                                    ) }}"
                                    method="POST">
                                    ...
                                    <select
                                        ...
                                        <option
                                            value="Espèces"
                                            {{ $vente->mode_paiement === 'Espèces' ? 'selected' : '' }}>
                                            Espèces
                                        </option>

                                        <option
                                            value="Chèque"
                                            {{ $vente->mode_paiement === 'Chèque' ? 'selected' : '' }}>
                                            Chèque
                                        </option>

                                        <option
                                            value="Carte bancaire"
                                            {{ $vente->mode_paiement === 'Carte bancaire' ? 'selected' : '' }}>
                                            Carte bancaire
                                        </option>

                                        <option
                                            value="Mobile Money"
                                            {{ $vente->mode_paiement === 'Mobile Money' ? 'selected' : '' }}>
                                            Mobile Money
                                        </option>
